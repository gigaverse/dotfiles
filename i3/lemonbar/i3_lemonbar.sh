...
#   ### End Brightness Check, "BRI" ### }}}
    ...
    if [ $((cnt_mail++)) -ge ${upd_mail} ]; then
        printf "%s%s\n" "EMA" "$(find ${HOME}/.mail/*/INBOX/new -type f 2>/dev/null | wc -l)" | tee "${panel_fifo[@]}" > /dev/null 
        cnt_mail=0
    fi
    ### End Offlineimap, "EMA" ### }}}
    ...
    if [ $((cnt_disk++)) -ge ${upd_disk} ]; then
        ...
        printf "%s%s\n" "DIC" "$(df --output=pcent / | awk 'END {print $1}')" | tee "${panel_fifo[@]}" > /dev/null 
        cnt_disk=0
    fi
